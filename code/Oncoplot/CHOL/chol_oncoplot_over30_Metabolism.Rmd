...
绘制图形
```{r CHOL_oncoplot_over30_Metabolism}
oncoplot(
  maf = maf,
  top = 50,
...
top3_genes <- top3_genes %>%
  mutate(Point_Mutation = Missense_Mutation,
         mutation_freq = round(MutatedSamples / n_samples,2)) %>%
  select(Hugo_Symbol,MutatedSamples,mutation_freq,total,Point_Mutation,Frame_Shift_Del)
print(top3_genes)
```
