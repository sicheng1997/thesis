...
加载TCGA-THYM.Rdata数据
```{r}
load("TCGA-THYM_data.Rdata")
load("~/Project/data/GWAS_Geneset.Rdata")
```
...
选取BMI >= 30的患者
```{r}
clinical_selected <- clinical_selected[clinical_selected$BMI >= 30,]
```
...
snv_data_table <- snv_data_table[snv_data_table$Hugo_Symbol %in% GWAS_geneset,]
```
...
top3_genes <- top3_genes %>%
  mutate(Point_Mutation = Missense_Mutation + Nonsense_Mutation ,#+ Nonstop_Mutation,
         mutation_freq = round(MutatedSamples / n_samples,2)) %>%
  select(Hugo_Symbol,MutatedSamples,mutation_freq,total,Point_Mutation,Frame_Shift_Del)#Frame_Shift_Ins,
print(top3_genes)
```
