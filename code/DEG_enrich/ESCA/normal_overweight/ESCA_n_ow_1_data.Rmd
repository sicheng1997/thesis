...
load("~/Rproject/TCGA/TCGA-ESCA/TCGA-ESCA_data.Rdata")
exp = assay(exp)
```
...
clinical = Clinical[Clinical$disease == "ESCA" & Clinical$BMI < 30,]
clinical[1:4,1:4]
...
```{r}
Group = ifelse(str_sub(colnames(exp),1,12) %in% str_sub(clinical$id[clinical$BMI < 25],1,12),"normal","overweight")
Group = factor(Group, levels = c("normal", "overweight"))
table(Group)
```
