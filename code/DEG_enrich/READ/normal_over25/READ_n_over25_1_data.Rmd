...
clinical = Clinical[Clinical$disease == "READ",]
clinical[1:4,1:4]
...
```{r}
Group = ifelse(str_sub(colnames(exp),1,12) %in% str_sub(clinical$id[clinical$BMI < 25],1,12),"normal","over25")
Group = factor(Group, levels = c("normal", "over25"))
table(Group)
```
